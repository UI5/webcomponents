...
import CustomLabel from "../../../_samples/main/ComboBox/CustomLabel/CustomLabel.md";
...
Use the `label` slot to display custom content in the input area when an item is selected — for example, to show a flag alongside the country name.
Listen to the `selection-change` event and update the slot content accordingly.
