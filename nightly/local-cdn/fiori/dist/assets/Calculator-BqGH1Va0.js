const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/Calculator-FzVgNuVV.js","assets/bundle.esm-CsF5OVjH.js","assets/Calculator-tpZwJqPg.js"])))=>i.map(i=>d[i]);
import{ag as t,ah as r,u as o}from"./bundle.esm-CsF5OVjH.js";const e=`<svg width="169" height="160" viewBox="0 0 169 160" fill="none" xmlns="http://www.w3.org/2000/svg" id="tnt-Dialog-Calculator">
<circle cx="80" cy="80" r="80" fill="var(--sapContent_Illustrative_Color7)"/>
<path d="M52.541 126.664C52.541 125.736 49.8464 124.993 46.2461 124.824C46.0402 121.229 45.2477 118.555 44.3193 118.557C43.3909 118.559 42.619 121.253 42.4213 124.868C38.8107 125.115 36.1264 125.921 36.1387 126.852C36.1511 127.782 38.8333 128.525 42.4337 128.692C42.6498 132.288 43.4321 134.961 44.3584 134.959C45.2847 134.957 46.0608 132.263 46.2584 128.649C49.8691 128.402 52.5513 127.595 52.541 126.664Z" fill="var(--sapContent_Illustrative_Color18)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M29.1699 26.2813L25.3054 34.7374L25.3336 34.7359C24.3973 36.3615 24.2477 38.419 25.11 40.4697L63.417 131.56C65.0544 135.453 69.72 137.848 73.838 136.908L139.04 122.024C141.233 121.523 142.828 120.178 143.593 118.443L143.598 118.447L147.916 108.745L142.326 110.405L105.225 22.1817C103.587 18.2882 98.9217 15.8938 94.8038 16.8338L31.011 31.3959L29.1699 26.2813Z" fill="var(--sapContent_Illustrative_Color13)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M136.981 97.6947L87.6853 84.0825L73.0605 137.046C73.3214 137.012 73.581 136.967 73.8385 136.908L139.041 122.024C141.233 121.524 142.829 120.179 143.594 118.443L143.599 118.448L147.917 108.746L142.327 110.405L136.981 97.6947Z" fill="var(--sapContent_Illustrative_Color14)"/>
<rect width="82.1755" height="114.113" rx="7.64797" transform="matrix(0.974922 -0.222547 0.387655 0.921804 26.3027 24.2632)" fill="var(--sapContent_Illustrative_Color2)"/>
<path d="M72.4394 62.9946L71.8611 64.5562L70.8486 63.8331L72.4394 62.9946Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 49.5156 58.2085)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M51.4275 57.3127L50.8057 59.3239L52.537 59.3239L51.4275 57.3127Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 50.1738 56.2327)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M79.2021 79.5833L78.6238 81.1448L77.6113 80.4217L79.2021 79.5833Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 56.2773 74.7969)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M58.1902 73.9011L57.5684 75.9123L59.2997 75.9123L58.1902 73.9011Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 56.9365 72.821)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M86.4902 96.1711L85.9119 97.7327L84.8994 97.0096L86.4902 96.1711Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 63.5654 91.3848)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M65.4783 90.489L64.8564 92.5002L66.5877 92.5002L65.4783 90.489Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 64.2246 89.4089)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M93.373 112.837L92.7947 114.398L91.7822 113.675L93.373 112.837Z" fill="var(--sapContent_Illustrative_Color17)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 70.4482 108.051)" fill="var(--sapContent_Illustrative_Color17)"/>
<path d="M72.3611 107.155L71.7393 109.166L73.4706 109.166L72.3611 107.155Z" fill="var(--sapContent_Illustrative_Color17)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 71.1074 106.075)" fill="var(--sapContent_Illustrative_Color9)"/>
<path d="M116.997 107.554L116.419 109.115L115.406 108.392L116.997 107.554Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 94.0723 102.767)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M95.9851 101.872L95.3633 103.883L97.0946 103.883L95.9851 101.872Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 94.7314 100.792)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M140.039 102.017L139.461 103.578L138.448 102.855L140.039 102.017Z" fill="var(--sapContent_Illustrative_Color17)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 117.114 97.2302)" fill="var(--sapContent_Illustrative_Color17)"/>
<path d="M119.027 96.3345L118.405 98.3457L120.137 98.3457L119.027 96.3345Z" fill="var(--sapContent_Illustrative_Color17)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 117.773 95.2544)" fill="var(--sapContent_Illustrative_Color9)"/>
<path d="M109.786 90.7998L109.208 92.3614L108.195 91.6382L109.786 90.7998Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 86.8613 86.0137)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M88.7742 85.1179L88.1523 87.1291L89.8836 87.1291L88.7742 85.1179Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 87.5205 84.0378)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M133.41 85.594L132.832 87.1556L131.819 86.4324L133.41 85.594Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 110.485 80.8076)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M112.398 79.9119L111.776 81.9231L113.508 81.9231L112.398 79.9119Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 111.145 78.8318)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M102.827 74.2119L102.249 75.7735L101.236 75.0504L102.827 74.2119Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 79.9023 69.4255)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M81.8152 68.5298L81.1934 70.541L82.9247 70.541L81.8152 68.5298Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 80.5615 67.4497)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M126.451 69.2715L125.873 70.8331L124.86 70.1099L126.451 69.2715Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 103.526 64.4851)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M105.439 63.5894L104.817 65.6006L106.549 65.6006L105.439 63.5894Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 104.186 62.5093)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M96.0634 57.6238L95.4852 59.1854L94.4727 58.4622L96.0634 57.6238Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 73.1387 52.8374)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M75.0515 51.9417L74.4297 53.9529L76.161 53.9529L75.0515 51.9417Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 73.7979 50.8616)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M119.688 52.2522L119.11 53.8138L118.098 53.0906L119.688 52.2522Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 96.7637 47.4658)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M98.6765 46.5701L98.0547 48.5813L99.786 48.5813L98.6765 46.5701Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="18.8317" height="13.1608" rx="3.52983" transform="matrix(0.974922 -0.222547 0.387655 0.921804 97.4229 45.49)" fill="var(--sapContent_Illustrative_Color8)"/>
<rect width="67.8419" height="24.5935" rx="4.70645" transform="matrix(0.974922 -0.222547 0.387655 0.921804 37.2227 28.5154)" fill="var(--sapContent_Illustrative_Color4)"/>
<rect width="67.1036" height="23.325" rx="4.70645" transform="matrix(0.974922 -0.222547 0.387655 0.921804 37.2158 29.6846)" fill="var(--sapContent_Illustrative_Color9)"/>
<path d="M81.707 38.0046L79.7506 37.0493L78.4659 33.9944L79.3726 32.4536L81.707 38.0046ZM74.399 34.0789C73.995 33.917 73.5884 33.7603 73.1792 33.6088L73.8349 32.7377L77.4205 31.9192L78.6167 32.3675L77.9845 33.2604L74.399 34.0789ZM72.4652 33.554L70.1308 28.003L72.0558 28.8838L73.351 31.9635L72.4652 33.554ZM76.816 26.4769L79.1504 32.0279L77.2254 31.1472L75.9354 28.0798L76.816 26.4769Z" fill="var(--sapContent_Illustrative_Color4)"/>
<path d="M92.2641 35.5947L90.3208 34.6364L89.0361 31.5815L89.9297 30.0437L92.2641 35.5947ZM80.8836 24.9632L86.7676 23.62L86.0445 25.255L82.8004 25.9955L80.8836 24.9632ZM87.3731 24.0671L89.7075 29.618L87.7956 28.7343L86.5057 25.6669L87.3731 24.0671Z" fill="var(--sapContent_Illustrative_Color4)"/>
<path d="M96.6548 35.1913L97.4435 33.5413L100.688 32.8008L102.526 33.8511L96.6548 35.1913ZM93.6965 29.1838L95.6581 30.1515L96.9376 33.194L96.0309 34.7348L93.6965 29.1838ZM95.4081 29.2831C95.0041 29.1213 94.5976 28.9645 94.1883 28.813L94.8441 27.9419L98.4296 27.1234L99.6258 27.5717L98.9936 28.4646L95.4081 29.2831ZM91.3357 22.5773L97.2065 21.2371L96.4703 22.8751L93.2525 23.6096L91.3357 22.5773ZM97.8251 21.6812L100.16 27.2321L98.2345 26.3514L96.9445 23.284L97.8251 21.6812Z" fill="var(--sapContent_Illustrative_Color4)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M66.6192 46.5374L73.7099 21.3537L81.1966 19.6447L74.1059 44.8284L66.6192 46.5374Z" fill="var(--sapContent_Illustrative_Color7)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M50.7617 50.1569L57.8535 24.9737L70.2666 22.1401L63.1746 47.3242L50.8466 50.1383C50.8183 50.1447 50.7901 50.1509 50.7617 50.1569Z" fill="var(--sapContent_Illustrative_Color7)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M78.2887 29.9785L76.8162 26.4771L75.9356 28.0799L77.2255 31.1473L77.8759 31.4448L78.2887 29.9785ZM77.7115 32.0284L77.4207 31.9193L73.8351 32.7378L73.1794 33.6089C73.5886 33.7604 73.9952 33.9172 74.3991 34.079L77.322 33.4118L77.7115 32.0284ZM71.1543 30.4364L72.4654 33.5541L73.3512 31.9636L72.056 28.8839L71.6444 28.6955L71.1543 30.4364Z" fill="var(--sapContent_Illustrative_Color9)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M114.182 13.5068L109.976 18.8249L109.997 18.8263C109.879 18.9597 109.769 19.1 109.666 19.2472C106.556 23.7173 112.017 32.8948 121.864 39.7456C131.382 46.3678 141.513 48.3922 144.988 44.4827L145.001 44.5134L149.27 38.7968L146.056 39.5713C145.184 34.8235 140.328 28.5685 133.126 23.558C126.816 19.1677 120.236 16.7983 115.505 16.839L114.182 13.5068Z" fill="var(--sapContent_Illustrative_Color3)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M131.157 22.2578L118.49 37.1695C110.821 30.7431 106.932 23.1763 109.666 19.247C109.769 19.0998 109.879 18.9595 109.997 18.8261L109.976 18.8246L114.182 13.5066L115.505 16.8388C119.746 16.8023 125.472 18.7021 131.157 22.2578Z" fill="var(--sapContent_Illustrative_Color15)"/>
<ellipse cx="131.685" cy="26.3109" rx="21.7197" ry="9.8601" transform="rotate(34.8285 131.685 26.3109)" fill="var(--sapContent_Illustrative_Color3)"/>
<ellipse cx="131.685" cy="25.7517" rx="16.8544" ry="6.05785" transform="rotate(34.8285 131.685 25.7517)" fill="var(--sapContent_Illustrative_Color15)"/>
<ellipse cx="16.5089" cy="5.92719" rx="16.5089" ry="5.92719" transform="matrix(0.821703 0.569916 -0.57233 0.820024 121.175 12.3713)" fill="var(--sapContent_Illustrative_Color3)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M135.984 35.8785L131.076 19.488C132.283 20.1627 133.513 20.929 134.74 21.78C142.232 26.9763 146.786 33.3648 144.913 36.0491C143.729 37.7447 140.264 37.5568 135.984 35.8785Z" fill="var(--sapContent_Illustrative_Color8)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M4.99333 101.129L8.93923 107.439L8.94716 107.422C9.04152 107.592 9.14554 107.757 9.25938 107.918C12.7244 112.815 23.9725 111.509 34.3827 105.001C44.4454 98.7109 50.0419 89.8631 47.2751 84.857L47.312 84.8582L42.9827 78.316L42.6494 81.7298C37.512 80.5323 29.4493 82.5093 21.8349 87.2691C15.1639 91.4392 10.4558 96.7333 8.846 101.287L4.99333 101.129Z" fill="var(--sapContent_Illustrative_Color15)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M14.6105 110.734L8.95439 100.993C9.86778 98.5891 11.6262 95.9928 14.066 93.4727L23.5814 109.861C20.2383 110.791 17.139 111.095 14.6105 110.734Z" fill="var(--sapContent_Illustrative_Color3)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M17.2046 90.5898L27.5952 108.485C29.839 107.574 32.131 106.409 34.3837 105.001C34.5474 104.899 34.7099 104.796 34.8711 104.692L24.3033 86.492L19.3377 88.9474C18.5951 89.4831 17.8834 90.0316 17.2046 90.5898Z" fill="var(--sapContent_Illustrative_Color3)"/>
<ellipse cx="22.2321" cy="12.0045" rx="22.2321" ry="12.0045" transform="matrix(0.846045 -0.533112 0.570518 0.821285 -1.61719 91.6626)" fill="var(--sapContent_Illustrative_Color3)"/>
<ellipse cx="17.2521" cy="7.37534" rx="17.2521" ry="7.37534" transform="matrix(0.846045 -0.533112 0.570518 0.821285 4.64355 92.5242)" fill="var(--sapContent_Illustrative_Color15)"/>
<ellipse cx="17.0123" cy="7.21152" rx="17.0123" ry="7.21152" transform="matrix(0.851202 -0.524838 0.578878 0.815414 5.53516 93.1626)" fill="var(--sapContent_Illustrative_Color3)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M20.2199 84.1092C20.152 84.1505 20.0841 84.1921 20.0163 84.2339C18.5242 85.1539 17.1498 86.1331 15.9162 87.1384L13.8798 100.796C15.0384 100.846 16.3593 100.718 17.7877 100.421L20.2199 84.1092ZM32.2315 93.2793C31.0588 94.2183 29.7639 95.1324 28.3654 95.9947C25.4177 97.8122 22.4218 99.1452 19.6982 99.9414L22.2297 82.9632C26.695 80.5849 31.0873 79.3453 34.2971 79.4256L32.2315 93.2793Z" fill="var(--sapContent_Illustrative_Color8)"/>
<rect x="152.163" y="117.202" width="45.1381" height="10.9215" transform="rotate(133.112 152.163 117.202)" fill="var(--sapContent_Illustrative_Color15)"/>
<rect x="149.63" y="119.91" width="11.1558" height="10.9215" transform="rotate(133.112 149.63 119.91)" fill="var(--sapContent_Illustrative_Color3)"/>
<rect x="138.664" y="131.622" width="7.15142" height="10.9215" transform="rotate(133.112 138.664 131.622)" fill="var(--sapContent_Illustrative_Color3)"/>
<path d="M150.801 62.4285C150.801 61.8096 149.005 61.3143 146.604 61.2017C146.467 58.8045 145.939 57.0221 145.32 57.0234C144.701 57.0248 144.186 58.821 144.055 61.2306C141.648 61.3952 139.858 61.9331 139.866 62.5533C139.874 63.1736 141.663 63.6689 144.063 63.7801C144.207 66.1773 144.728 67.9597 145.346 67.9584C145.964 67.957 146.481 66.1608 146.613 63.7513C149.02 63.5866 150.808 63.0487 150.801 62.4285Z" fill="var(--sapContent_Illustrative_Color18)"/>
</svg>
`,i=`<svg width="321" height="240" viewBox="0 0 321 240" fill="none" xmlns="http://www.w3.org/2000/svg" id="tnt-Scene-Calculator">
<path d="M305.711 20H14.6174C12.72 19.9983 10.8408 20.3705 9.08735 21.0954C7.33387 21.8203 5.74044 22.8836 4.39815 24.2247C3.05586 25.5657 1.99103 27.1582 1.26452 28.911C0.538005 30.6638 0.164062 32.5426 0.164063 34.44V205.547C0.164063 209.38 1.68681 213.056 4.39734 215.767C7.10786 218.477 10.7841 220 14.6174 220H305.711C309.544 220 313.22 218.477 315.931 215.767C318.641 213.056 320.164 209.38 320.164 205.547V34.44C320.164 32.5426 319.79 30.6638 319.064 28.911C318.337 27.1582 317.272 25.5657 315.93 24.2247C314.588 22.8836 312.994 21.8203 311.241 21.0954C309.487 20.3705 307.608 19.9983 305.711 20Z" fill="var(--sapContent_Illustrative_Color7)"/>
<path d="M153.421 79.5163L165.799 65.0286C165.975 64.8241 166.199 64.7078 166.449 64.7089L176.445 64.7527C176.794 64.7542 176.916 65.2825 176.616 65.5157L150.531 84.7516L153.396 79.5455L153.421 79.5163Z" fill="var(--sapContent_Illustrative_Color19)"/>
<path d="M166.047 64.8535L100.041 103.118C86.8455 110.771 77.0621 124.713 73.1998 141.408L66.6695 169.642C64.9162 177.227 61.794 184.309 57.5294 190.388L48.2479 203.6L117.676 186.313C141.135 180.464 156.359 153.645 151.919 125.92C150.924 119.759 150.802 113.455 151.552 107.243L151.862 104.635C152.768 97.0162 154.322 89.4882 156.473 82.2267C157.51 78.7717 159.02 75.5239 160.927 72.6298L166.047 64.8535Z" fill="var(--sapContent_Illustrative_Color18)"/>
<path d="M125.667 183.357C126.142 183.154 121.716 186.184 106.257 192.537C80.6022 204.621 53.2281 210.775 25.6072 210.654C22.89 210.642 20.1994 210.249 17.5851 209.505L11.5597 207.808C11.1863 207.689 11.2139 207.073 11.5632 207.016L49.7451 197.772C53.5134 196.85 57.2331 195.635 60.8795 194.097L96.6687 179.038C97.4679 178.719 98.3401 178.781 99.0858 179.283C100.005 180.02 101.297 180.964 102.914 181.85C113.931 187.791 124.643 183.792 125.692 183.357L125.667 183.357Z" fill="var(--sapContent_Illustrative_Color19)"/>
<path d="M133.965 99.2205L88.0722 127.341C86.8474 128.098 86.3661 129.855 86.9829 131.324C87.5997 132.792 89.1176 133.414 90.3922 132.687L136.754 105.712C138.003 104.985 138.535 103.198 137.918 101.701L137.449 100.555C136.833 99.0571 135.265 98.4639 133.99 99.2206L133.965 99.2205Z" fill="var(--sapContent_Illustrative_Color19)"/>
<path d="M130.97 117.326L85.0774 145.447C83.8525 146.204 83.3463 147.961 83.988 149.43C84.6048 150.898 86.1227 151.52 87.3973 150.793L133.759 123.818C135.008 123.091 135.54 121.304 134.923 119.807L134.454 118.661C133.838 117.163 132.27 116.57 130.995 117.327L130.97 117.326Z" fill="var(--sapContent_Illustrative_Color19)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M82.2069 39.7037L76.3396 52.5422L76.3826 52.5398C74.9609 55.008 74.7338 58.1318 76.0431 61.2451L134.202 199.542C136.688 205.454 143.772 209.089 150.024 207.662L249.017 185.064C252.346 184.304 254.768 182.263 255.93 179.627L255.937 179.635L262.493 164.904L254.006 167.424L197.677 33.4795C195.191 27.5681 188.107 23.9329 181.855 25.3601L85.0022 47.4689L82.2069 39.7037Z" fill="var(--sapContent_Illustrative_Color13)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M245.889 148.127L171.046 127.46L148.842 207.871C149.238 207.82 149.632 207.751 150.023 207.662L249.016 185.064C252.345 184.305 254.767 182.263 255.929 179.627L255.936 179.635L262.492 164.905L254.005 167.424L245.889 148.127Z" fill="var(--sapContent_Illustrative_Color14)"/>
<rect width="124.763" height="173.252" rx="11.6115" transform="matrix(0.974922 -0.222547 0.387655 0.921804 77.8535 36.6399)" fill="var(--sapContent_Illustrative_Color2)"/>
<path d="M147.899 95.4434L147.021 97.8142L145.483 96.7163L147.899 95.4434Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 113.094 88.1765)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M115.998 86.8167L115.054 89.8701L117.682 89.8701L115.998 86.8167Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 114.094 85.1768)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M158.167 120.629L157.289 123L155.752 121.902L158.167 120.629Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 123.362 113.362)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M126.266 112.002L125.322 115.055L127.951 115.055L126.266 112.002Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 124.362 110.362)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M169.232 145.813L168.354 148.184L166.816 147.086L169.232 145.813Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 134.427 138.547)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M137.331 137.187L136.387 140.24L139.015 140.24L137.331 137.187Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 135.427 135.547)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M179.681 171.116L178.803 173.487L177.266 172.389L179.681 171.116Z" fill="var(--sapContent_Illustrative_Color17)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 144.876 163.849)" fill="var(--sapContent_Illustrative_Color17)"/>
<path d="M147.78 162.49L146.836 165.543L149.464 165.543L147.78 162.49Z" fill="var(--sapContent_Illustrative_Color17)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 145.876 160.85)" fill="var(--sapContent_Illustrative_Color9)"/>
<path d="M215.549 163.095L214.671 165.466L213.134 164.368L215.549 163.095Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 180.744 155.828)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M183.648 154.469L182.704 157.522L185.333 157.522L183.648 154.469Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 181.744 152.829)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M250.532 154.688L249.654 157.059L248.117 155.961L250.532 154.688Z" fill="var(--sapContent_Illustrative_Color17)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 215.728 147.422)" fill="var(--sapContent_Illustrative_Color17)"/>
<path d="M218.632 146.062L217.688 149.115L220.316 149.115L218.632 146.062Z" fill="var(--sapContent_Illustrative_Color17)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 216.728 144.422)" fill="var(--sapContent_Illustrative_Color9)"/>
<path d="M204.6 137.658L203.722 140.029L202.185 138.931L204.6 137.658Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 169.795 130.392)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M172.699 129.032L171.755 132.085L174.383 132.085L172.699 129.032Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 170.795 127.392)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M240.468 129.755L239.59 132.126L238.053 131.028L240.468 129.755Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 205.663 122.488)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M208.567 121.128L207.623 124.182L210.252 124.182L208.567 121.128Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 206.663 119.488)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M194.035 112.474L193.157 114.844L191.62 113.747L194.035 112.474Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 159.23 105.207)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M162.135 103.847L161.19 106.9L163.819 106.9L162.135 103.847Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 160.23 102.207)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M229.903 104.973L229.026 107.344L227.488 106.246L229.903 104.973Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 195.099 97.7061)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M198.003 96.3462L197.059 99.3997L199.687 99.3997L198.003 96.3462Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 196.099 94.7063)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M183.767 87.2888L182.889 89.6597L181.352 88.5618L183.767 87.2888Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 148.962 80.022)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M151.866 78.6621L150.922 81.7156L153.55 81.7156L151.866 78.6621Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 149.962 77.0222)" fill="var(--sapContent_Illustrative_Color8)"/>
<path d="M219.635 79.1338L218.757 81.5046L217.22 80.4068L219.635 79.1338Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 184.83 71.8669)" fill="var(--sapContent_Illustrative_Color6)"/>
<path d="M187.734 70.5071L186.79 73.5606L189.419 73.5606L187.734 70.5071Z" fill="var(--sapContent_Illustrative_Color6)"/>
<rect width="28.5912" height="19.9813" rx="5.35915" transform="matrix(0.974922 -0.222547 0.387655 0.921804 185.83 68.8672)" fill="var(--sapContent_Illustrative_Color8)"/>
<rect width="103.001" height="37.339" rx="7.14554" transform="matrix(0.974922 -0.222547 0.387655 0.921804 94.4326 43.0955)" fill="var(--sapContent_Illustrative_Color4)"/>
<rect width="101.88" height="35.413" rx="7.14554" transform="matrix(0.974922 -0.222547 0.387655 0.921804 94.4219 44.8708)" fill="var(--sapContent_Illustrative_Color9)"/>
<path d="M161.97 57.5027L159 56.0523L157.049 51.4142L158.426 49.0749L161.97 57.5027ZM150.875 51.5425C150.261 51.2968 149.644 51.0589 149.023 50.8287L150.018 49.5063L155.462 48.2636L157.278 48.9443L156.318 50.2999L150.875 51.5425ZM147.939 50.7456L144.395 42.3178L147.317 43.655L149.284 48.3309L147.939 50.7456ZM154.544 40.0009L158.089 48.4287L155.166 47.0914L153.207 42.4345L154.544 40.0009Z" fill="var(--sapContent_Illustrative_Color4)"/>
<path d="M177.998 53.8439L175.048 52.389L173.098 47.7509L174.454 45.4161L177.998 53.8439ZM160.72 37.7026L169.653 35.6634L168.556 38.1457L163.63 39.27L160.72 37.7026ZM170.573 36.3421L174.117 44.7699L171.214 43.4281L169.256 38.7711L170.573 36.3421Z" fill="var(--sapContent_Illustrative_Color4)"/>
<path d="M184.665 53.2314L185.862 50.7263L190.787 49.602L193.578 51.1967L184.665 53.2314ZM180.173 44.1106L183.151 45.5798L185.094 50.1991L183.717 52.5384L180.173 44.1106ZM182.772 44.2613C182.158 44.0156 181.541 43.7777 180.92 43.5476L181.915 42.2251L187.359 40.9825L189.175 41.6631L188.216 43.0187L182.772 44.2613ZM176.589 34.0802L185.502 32.0455L184.384 34.5324L179.499 35.6476L176.589 34.0802ZM186.441 32.7197L189.986 41.1475L187.063 39.8102L185.104 35.1533L186.441 32.7197Z" fill="var(--sapContent_Illustrative_Color4)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M139.063 70.4576L149.828 32.2225L161.195 29.6279L150.43 67.8629L139.063 70.4576Z" fill="var(--sapContent_Illustrative_Color7)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M114.987 75.9525L125.754 37.7183L144.6 33.4163L133.833 71.6518L115.117 75.9242C115.074 75.9341 115.03 75.9435 114.987 75.9525Z" fill="var(--sapContent_Illustrative_Color7)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M156.779 45.3148L154.544 40.001L153.207 42.4345L155.166 47.0915L156.152 47.5426L156.779 45.3148ZM155.902 48.4287L155.462 48.2637L150.018 49.5064L149.023 50.8288C149.644 51.0589 150.261 51.2969 150.875 51.5426L155.311 50.53L155.902 48.4287ZM145.947 46.0099L147.939 50.7456L149.284 48.3309L147.317 43.6551L146.691 43.3685L145.947 46.0099Z" fill="var(--sapContent_Illustrative_Color9)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M219.616 33.2537L213.26 41.292L213.291 41.2942C213.112 41.4958 212.945 41.7077 212.791 41.9302C208.09 48.6869 216.344 62.5588 231.228 72.9141C245.614 82.9234 260.928 85.9834 266.18 80.0749L266.199 80.1211L272.653 71.4802L267.795 72.6507C266.476 65.4744 259.137 56.0197 248.251 48.4461C238.714 41.8101 228.768 38.2287 221.617 38.2901L219.616 33.2537Z" fill="var(--sapContent_Illustrative_Color3)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M245.275 46.4809L226.128 69.0202C214.536 59.3066 208.658 47.8695 212.791 41.9302C212.945 41.7077 213.112 41.4958 213.291 41.2942L213.26 41.292L219.616 33.2537L221.617 38.2901C228.027 38.235 236.681 41.1065 245.275 46.4809Z" fill="var(--sapContent_Illustrative_Color15)"/>
<ellipse cx="246.072" cy="52.6074" rx="32.8299" ry="14.9038" transform="rotate(34.8285 246.072 52.6074)" fill="var(--sapContent_Illustrative_Color3)"/>
<ellipse cx="246.072" cy="51.7623" rx="25.4759" ry="9.1566" transform="rotate(34.8285 246.072 51.7623)" fill="var(--sapContent_Illustrative_Color15)"/>
<ellipse cx="24.9537" cy="8.95911" rx="24.9537" ry="8.95911" transform="matrix(0.821703 0.569916 -0.57233 0.820024 230.186 31.5376)" fill="var(--sapContent_Illustrative_Color3)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M252.572 67.0702L245.154 42.2959C246.978 43.3154 248.836 44.4733 250.69 45.7592C262.014 53.6135 268.899 63.2699 266.067 67.3273C264.278 69.8901 259.041 69.6062 252.572 67.0702Z" fill="var(--sapContent_Illustrative_Color8)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M22.4338 150.811L28.3982 160.349L28.4109 160.321C28.5538 160.579 28.7114 160.829 28.8839 161.073C34.1213 168.474 51.1231 166.5 66.8585 156.664C82.0684 147.156 90.5278 133.783 86.3457 126.216L86.4005 126.218L79.8566 116.329L79.3528 121.489C71.5874 119.679 59.401 122.667 47.8921 129.862C37.8088 136.165 30.6923 144.167 28.259 151.05L22.4338 150.811Z" fill="var(--sapContent_Illustrative_Color15)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M36.9728 165.33L28.4231 150.606C29.8039 146.972 32.4618 143.048 36.1496 139.239L50.5326 164.01C45.4794 165.416 40.7947 165.875 36.9728 165.33Z" fill="var(--sapContent_Illustrative_Color3)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M40.8928 134.881L56.5985 161.93C59.99 160.553 63.4542 158.793 66.8592 156.664C67.1067 156.51 67.3524 156.354 67.5963 156.197L51.6229 128.687L44.1151 132.4C42.9934 133.209 41.9183 134.038 40.8928 134.881Z" fill="var(--sapContent_Illustrative_Color3)"/>
<ellipse cx="33.6044" cy="18.1452" rx="33.6044" ry="18.1452" transform="matrix(0.846045 -0.533112 0.570518 0.821285 12.4443 136.503)" fill="var(--sapContent_Illustrative_Color3)"/>
<ellipse cx="26.077" cy="11.148" rx="26.077" ry="11.148" transform="matrix(0.846045 -0.533112 0.570518 0.821285 21.9062 137.805)" fill="var(--sapContent_Illustrative_Color15)"/>
<ellipse cx="25.7146" cy="10.9004" rx="25.7146" ry="10.9004" transform="matrix(0.851202 -0.524838 0.578878 0.815414 23.2539 138.77)" fill="var(--sapContent_Illustrative_Color3)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M45.4515 125.085C45.3483 125.147 45.2452 125.21 45.1421 125.274C42.8874 126.664 40.8106 128.144 38.9464 129.663L35.8681 150.308C37.6194 150.384 39.616 150.191 41.7751 149.741L45.4515 125.085ZM63.6069 138.945C61.8341 140.365 59.8763 141.747 57.7621 143.051C53.3069 145.798 48.7789 147.813 44.6624 149.016L48.4889 123.353C55.2384 119.758 61.8775 117.885 66.729 118.006L63.6069 138.945Z" fill="var(--sapContent_Illustrative_Color8)"/>
<rect x="282.581" y="169.955" width="68.2275" height="16.5082" transform="rotate(133.112 282.581 169.955)" fill="var(--sapContent_Illustrative_Color15)"/>
<rect x="278.75" y="174.048" width="16.8623" height="16.5082" transform="rotate(133.112 278.75 174.048)" fill="var(--sapContent_Illustrative_Color3)"/>
<rect x="262.176" y="191.751" width="10.8096" height="16.5082" transform="rotate(133.112 262.176 191.751)" fill="var(--sapContent_Illustrative_Color3)"/>
</svg>
`,n=`<svg width="128" height="128" version="1.1" id="tnt-Spot-Calculator" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 128 128" enable-background="new 0 0 128 128" xml:space="preserve">
<circle fill="var(--sapContent_Illustrative_Color7)" cx="64" cy="64" r="64"/>
<path fill="var(--sapContent_Illustrative_Color18)" d="M39.2,107.5c0-0.8-2.3-1.4-5.3-1.6c-0.2-3-0.8-5.3-1.6-5.3s-1.4,2.3-1.6,5.3c-3,0.2-5.3,0.9-5.3,1.7
	c0,0.8,2.3,1.4,5.3,1.6c0.2,3,0.8,5.3,1.6,5.3s1.4-2.3,1.6-5.3C37,108.9,39.3,108.3,39.2,107.5z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color13)" d="M18.6,22.2l-3.3,7.2l0,0c-0.8,1.4-0.9,3.1-0.2,4.9l32.5,77.3
	c1.4,3.3,5.4,5.3,8.8,4.5l55.4-12.6c1.9-0.4,3.2-1.6,3.9-3l0,0l3.7-8.2l-4.7,1.4L83.2,18.8c-1.4-3.3-5.4-5.3-8.8-4.5L20.2,26.6
	L18.6,22.2z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color14)" d="M110.2,82.9L68.3,71.3l-12.4,45c0.2,0,0.4-0.1,0.7-0.1l55.4-12.6
	c1.9-0.4,3.2-1.6,3.9-3l0,0l3.7-8.2l-4.7,1.4L110.2,82.9z"/>
<path fill="var(--sapContent_Illustrative_Color2)" d="M22.5,19.1L77.9,6.4c3.5-0.8,7.5,1.2,8.8,4.5l32.5,77.3c1.4,3.3-0.3,6.6-3.8,7.4L60,108.2
	c-3.5,0.8-7.5-1.2-8.8-4.5L18.7,26.5C17.3,23.2,19,19.9,22.5,19.1z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M55.4,53.4l-0.5,1.3L54,54.1L55.4,53.4z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M38.8,48.7l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4L43.2,59
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C36.4,50.6,37.2,49,38.8,48.7z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M37.5,48.6L37,50.3h1.5L37.5,48.6z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M39.4,47l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2c-1.6,0.4-3.4-0.6-4.1-2.1
	l-2-4.8C37,48.9,37.8,47.4,39.4,47z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M61.1,67.5l-0.5,1.3l-0.9-0.6L61.1,67.5z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M44.6,62.8l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C42.2,64.7,43,63.1,44.6,62.8z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M43.3,62.7l-0.5,1.7h1.5L43.3,62.7z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M45.1,61.1l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C42.7,63,43.5,61.4,45.1,61.1z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M67.3,81.6l-0.5,1.3L66,82.3L67.3,81.6z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M50.8,76.8l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C48.4,78.7,49.1,77.2,50.8,76.8z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M49.5,76.7L49,78.4h1.5L49.5,76.7z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M51.3,75.2L61,73c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2c-1.6,0.4-3.4-0.6-4.1-2.1
	l-2-4.8C48.9,77.1,49.7,75.5,51.3,75.2z"/>
<path fill="var(--sapContent_Illustrative_Color17)" d="M73.1,95.7L72.7,97l-0.9-0.6L73.1,95.7z"/>
<path fill="var(--sapContent_Illustrative_Color17)" d="M56.6,91l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2c-1.6,0.4-3.4-0.6-4.1-2.1
	l-2-4.8C54.2,92.9,55,91.4,56.6,91z"/>
<path fill="var(--sapContent_Illustrative_Color17)" d="M55.3,90.9l-0.5,1.7h1.5L55.3,90.9z"/>
<path fill="var(--sapContent_Illustrative_Color9)" d="M57.2,89.3l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C54.8,91.2,55.6,89.7,57.2,89.3z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M93.2,91.2l-0.5,1.3l-0.9-0.6L93.2,91.2z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M76.7,86.5l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4L81,96.8
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C74.3,88.4,75.1,86.9,76.7,86.5z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M75.4,86.4l-0.5,1.7h1.5L75.4,86.4z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M77.2,84.8l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C74.8,86.7,75.6,85.2,77.2,84.8z"/>
<path fill="var(--sapContent_Illustrative_Color17)" d="M112.8,86.5l-0.5,1.3l-0.9-0.6L112.8,86.5z"/>
<path fill="var(--sapContent_Illustrative_Color17)" d="M96.2,81.8l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C93.8,83.7,94.6,82.2,96.2,81.8z"/>
<path fill="var(--sapContent_Illustrative_Color17)" d="M94.9,81.7l-0.5,1.7h1.5L94.9,81.7z"/>
<path fill="var(--sapContent_Illustrative_Color9)" d="M96.8,80.1l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C94.4,82,95.2,80.5,96.8,80.1z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M87.1,77l-0.5,1.3l-0.9-0.6L87.1,77z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M70.5,72.3l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C68.1,74.2,68.9,72.6,70.5,72.3z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M69.2,72.2l-0.5,1.7h1.5L69.2,72.2z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M71.1,70.6l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C68.7,72.5,69.5,71,71.1,70.6z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M107.1,72.6l-0.5,1.3l-0.9-0.6L107.1,72.6z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M90.6,67.9l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C88.2,69.8,89,68.2,90.6,67.9z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M89.3,67.8l-0.5,1.7h1.5L89.3,67.8z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M91.2,66.2l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C88.8,68.1,89.5,66.5,91.2,66.2z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M81.2,62.9l-0.5,1.3l-0.9-0.6L81.2,62.9z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M64.6,58.2l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4L69,68.5
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C62.2,60.1,63,58.6,64.6,58.2z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M63.3,58.1l-0.5,1.7h1.5L63.3,58.1z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M65.2,56.5l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C62.8,58.4,63.6,56.9,65.2,56.5z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M101.2,58.7l-0.5,1.3l-0.9-0.6L101.2,58.7z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M84.7,54l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4L89,64.3c-1.6,0.4-3.4-0.6-4.1-2.1
	l-2-4.8C82.3,55.9,83.1,54.4,84.7,54z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M83.4,53.9l-0.5,1.7h1.5L83.4,53.9z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M85.2,52.3l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C82.8,54.2,83.6,52.7,85.2,52.3z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M75.4,48.8l-0.5,1.3L74,49.5L75.4,48.8z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M58.9,44.1l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C56.5,46,57.3,44.5,58.9,44.1z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M57.6,44l-0.5,1.7h1.5L57.6,44z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M59.5,42.4l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C57,44.3,57.8,42.8,59.5,42.4z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M95.5,44.3L95,45.6L94.1,45L95.5,44.3z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M78.9,39.6l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C76.5,41.5,77.3,39.9,78.9,39.6z"/>
<path fill="var(--sapContent_Illustrative_Color6)" d="M77.7,39.5l-0.5,1.7h1.5L77.7,39.5z"/>
<path fill="var(--sapContent_Illustrative_Color8)" d="M79.5,37.9l9.7-2.2c1.6-0.4,3.4,0.6,4.1,2.1l2,4.8c0.6,1.5-0.1,3.1-1.8,3.4l-9.7,2.2
	c-1.6,0.4-3.4-0.6-4.1-2.1l-2-4.8C77.1,39.8,77.9,38.2,79.5,37.9z"/>
<path fill="var(--sapContent_Illustrative_Color4)" d="M29.4,23.2l48.4-11c2.2-0.5,4.6,0.8,5.4,2.8l5,11.9c0.9,2-0.2,4.1-2.3,4.6l-48.4,11
	c-2.2,0.5-4.6-0.8-5.4-2.8l-5-11.9C26.2,25.8,27.2,23.7,29.4,23.2z"/>
<path fill="var(--sapContent_Illustrative_Color9)" d="M29.4,24.2l47.8-10.9c2.2-0.5,4.6,0.8,5.4,2.8L87.1,27c0.9,2-0.2,4.1-2.3,4.6L37,42.5
	c-2.2,0.5-4.6-0.8-5.4-2.8L27,28.8C26.2,26.8,27.2,24.7,29.4,24.2z"/>
<path fill="var(--sapContent_Illustrative_Color4)" d="M63.2,32.2l-1.7-0.8l-1.1-2.6l0.8-1.3L63.2,32.2z M57,28.9c-0.3-0.1-0.7-0.3-1-0.4l0.6-0.7l3-0.7l1,0.4
	l-0.5,0.8L57,28.9z M55.4,28.4l-2-4.7l1.6,0.7l1.1,2.6L55.4,28.4z M59.1,22.4l2,4.7l-1.6-0.7l-1.1-2.6L59.1,22.4z"/>
<path fill="var(--sapContent_Illustrative_Color4)" d="M72.2,30.1l-1.6-0.8l-1.1-2.6l0.8-1.3L72.2,30.1z M62.5,21.1l5-1.1l-0.6,1.4L64.2,22L62.5,21.1z M68.1,20.4
	l2,4.7l-1.6-0.8l-1.1-2.6L68.1,20.4z"/>
<path fill="var(--sapContent_Illustrative_Color4)" d="M75.9,29.8l0.7-1.4l2.8-0.6l1.6,0.9L75.9,29.8z M73.4,24.7l1.7,0.8l1.1,2.6l-0.8,1.3L73.4,24.7z M74.9,24.8
	c-0.3-0.1-0.7-0.3-1-0.4l0.6-0.7l3-0.7l1,0.4L78,24.2L74.9,24.8z M71.4,19.1l5-1.1l-0.6,1.4L73,20L71.4,19.1z M76.9,18.3l2,4.7
	l-1.6-0.7l-1.1-2.6L76.9,18.3z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color7)" d="M50.4,39.4l6-21.4l6.4-1.5l-6,21.4L50.4,39.4z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color7)" d="M37,42.5l6-21.4l10.5-2.4l-6,21.4L37,42.5L37,42.5z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color9)" d="M60.3,25.4l-1.2-3l-0.7,1.4l1.1,2.6l0.6,0.3L60.3,25.4z M59.8,27.1
	L59.6,27l-3,0.7L56,28.5c0.3,0.1,0.7,0.3,1,0.4l2.5-0.6L59.8,27.1z M54.3,25.8l1.1,2.6l0.8-1.4L55,24.4l-0.4-0.2L54.3,25.8z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color3)" d="M120.3,33.5c0.1,0.1,0.1,0.3,0.2,0.4l-3.2,4.3l0,0
	c-2.9,3.3-11.5,1.6-19.5-4c-8.3-5.8-12.9-13.5-10.3-17.3c0.1-0.1,0.2-0.2,0.3-0.4l0,0l3.5-4.5l1.1,2.8c4,0,9.5,2,14.9,5.7
	c6.1,4.2,10.2,9.5,10.9,13.5L120.3,33.5z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color15)" d="M105.6,19.4L94.9,31.9c-6.5-5.4-9.8-11.8-7.4-15.1
	c0.1-0.1,0.2-0.2,0.3-0.4l0,0l3.5-4.5l1.1,2.8C96,14.8,100.8,16.4,105.6,19.4z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color3)" d="M99.4,10.7c8.8,5.9,16.1,13.9,21.1,23.2
	c-3.2,2.9-11.4,1.1-19.2-4.3C93,23.8,88.4,16.1,91,12.3C92.4,10.3,95.5,9.8,99.4,10.7z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color15)" d="M118,29.7c-0.1,0.3-0.2,0.5-0.3,0.7c-1.6,2.3-8.1,0.6-14.6-3.9
	c-6.4-4.5-10.4-10-8.8-12.3c1.5-2.2,7.4-0.8,13.4,3.1C111.7,21,115.1,25.1,118,29.7z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color3)" d="M111.7,21.3c2.1,2.3,4,4.8,5.7,7.4c0.3,0.9,0.2,1.7-0.2,2.3
	c-1.6,2.3-8,0.5-14.3-3.8c-6.3-4.4-10.2-9.8-8.6-12c1.6-2.3,8-0.5,14.3,3.8C109.7,19.7,110.8,20.5,111.7,21.3z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color8)" d="M111.7,21.4c2.1,2.3,4,4.8,5.7,7.4c0.3,0.9,0.2,1.7-0.2,2.3
	c-1,1.4-3.9,1.3-7.5-0.1l-4.2-14c1,0.6,2.1,1.2,3.1,1.9C109.7,19.7,110.8,20.5,111.7,21.4z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color15)" d="M3.2,84.1c1.8-3.4,5.4-7,10.1-9.9c6.4-4,13.2-5.7,17.6-4.7l0.3-2.9
	l3.7,5.5l0,0c2.3,4.2-2.4,11.7-10.9,17c-5.9,3.7-12.1,5.4-16.4,4.9C5.8,90.9,4.4,87.6,3.2,84.1z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color3)" d="M7.5,94.1c-1.7-3.2-3.1-6.5-4.3-9.9c0.8-1.5,2-3.1,3.5-4.7l8,13.8
	C12.1,94,9.6,94.3,7.5,94.1z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color3)" d="M9.4,77l8.8,15.1c1.9-0.8,3.8-1.8,5.7-2.9c0.1-0.1,0.3-0.2,0.4-0.3
	l-8.9-15.4l-4.2,2.1C10.6,76.1,10,76.6,9.4,77z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color3)" d="M1,75.6c1.9-2.7,4.8-5.4,8.4-7.7c8.8-5.5,18.5-6.3,21.7-1.7
	S29.7,79,21,84.6c-5.5,3.5-11.4,5.1-15.8,4.6C3.3,84.9,1.9,80.3,1,75.6z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color15)" d="M2.1,80.5c0.9-2.9,4.3-6.6,9-9.6c6.8-4.3,13.9-5.5,15.9-2.7
	s-2,8.6-8.8,12.9c-5.9,3.7-12,5.1-14.8,3.6C2.9,83.3,2.5,81.9,2.1,80.5z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color3)" d="M2.6,82.2c0.3-3,3.8-7.2,9.2-10.5c6.7-4.2,13.8-5.3,15.7-2.6
	c1.9,2.7-1.9,8.3-8.7,12.5c-6.5,4-13.3,5.2-15.5,2.9C3.1,83.7,2.8,82.9,2.6,82.2z"/>
<path fill-rule="evenodd" clip-rule="evenodd" fill="var(--sapContent_Illustrative_Color8)" d="M12,71.6c-0.1,0-0.1,0.1-0.2,0.1c-1.3,0.8-2.4,1.6-3.5,2.4
	L6.6,85.6c1,0,2.1-0.1,3.3-0.3L12,71.6z M22.1,79.3c-1,0.8-2.1,1.6-3.3,2.3c-2.5,1.5-5,2.7-7.3,3.3l2.1-14.3c3.8-2,7.5-3.1,10.2-3
	L22.1,79.3z"/>
<path fill="var(--sapContent_Illustrative_Color18)" d="M122.2,53.3c0-0.5-1.5-0.9-3.5-1c-0.1-2-0.6-3.5-1.1-3.5s-1,1.5-1.1,3.6c-2,0.1-3.5,0.6-3.5,1.1
	s1.5,0.9,3.5,1c0.1,2,0.6,3.5,1.1,3.5s1-1.5,1.1-3.6C120.7,54.2,122.2,53.8,122.2,53.3z"/>
</svg>
`,C=`<svg width="45" height="46" viewBox="0 0 45 46" fill="none" xmlns="http://www.w3.org/2000/svg" id="tnt-Dot-Calculator">
<rect x="5.2168" y="2.99585" width="33.2178" height="40.8286" rx="4" fill="var(--sapContent_Illustrative_Color5)"/>
<path d="M8.2168 4.95913C8.2168 3.28062 9.5775 1.91992 11.256 1.91992L36.744 1.91992C38.4225 1.91992 39.7832 3.28062 39.7832 4.95913V38.0895C39.7832 39.768 38.4225 41.1287 36.744 41.1287H11.256C9.5775 41.1287 8.2168 39.768 8.2168 38.0895L8.2168 4.95913Z" fill="var(--sapContent_Illustrative_Color8)"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M11.256 0.919922L36.744 0.919922C38.9748 0.919922 40.7832 2.72834 40.7832 4.95913V38.0895C40.7832 40.3202 38.9748 42.1287 36.744 42.1287H11.256C9.02521 42.1287 7.2168 40.3202 7.2168 38.0895L7.2168 4.95913C7.2168 2.72834 9.02521 0.919922 11.256 0.919922ZM11.256 1.91992C9.5775 1.91992 8.2168 3.28062 8.2168 4.95913L8.2168 38.0895C8.2168 39.768 9.5775 41.1287 11.256 41.1287H36.744C38.4225 41.1287 39.7832 39.768 39.7832 38.0895V4.95913C39.7832 3.28062 38.4225 1.91992 36.744 1.91992L11.256 1.91992Z" fill="var(--sapContent_Illustrative_Color4)"/>
<path d="M11.6846 7.09863C11.6846 5.99406 12.58 5.09863 13.6846 5.09863L34.3154 5.09863C35.42 5.09863 36.3154 5.99406 36.3154 7.09863V13.792C36.3154 14.8966 35.42 15.792 34.3154 15.792L13.6846 15.792C12.58 15.792 11.6846 14.8966 11.6846 13.792V7.09863Z" fill="var(--sapContent_Illustrative_Color17)"/>
<path d="M11.6846 22.2842C11.6846 21.1796 12.58 20.2842 13.6846 20.2842H16.0518C17.1564 20.2842 18.0518 21.1796 18.0518 22.2842V25.8056C18.0518 26.9101 17.1564 27.8056 16.0518 27.8056H13.6846C12.58 27.8056 11.6846 26.9101 11.6846 25.8056V22.2842Z" fill="var(--sapContent_Illustrative_Color5)"/>
<path d="M11.6846 32.095C11.6846 30.9904 12.58 30.095 13.6846 30.095H16.0518C17.1564 30.095 18.0518 30.9904 18.0518 32.095V35.6164C18.0518 36.7209 17.1564 37.6164 16.0518 37.6164H13.6846C12.58 37.6164 11.6846 36.7209 11.6846 35.6164V32.095Z" fill="var(--sapContent_Illustrative_Color5)"/>
<path d="M20.8164 22.2842C20.8164 21.1796 21.7118 20.2842 22.8164 20.2842H25.1836C26.2882 20.2842 27.1836 21.1796 27.1836 22.2842V25.8056C27.1836 26.9101 26.2882 27.8056 25.1836 27.8056H22.8164C21.7118 27.8056 20.8164 26.9101 20.8164 25.8056V22.2842Z" fill="var(--sapContent_Illustrative_Color5)"/>
<path d="M20.8164 32.095C20.8164 30.9904 21.7118 30.095 22.8164 30.095H25.1836C26.2882 30.095 27.1836 30.9904 27.1836 32.095V35.6164C27.1836 36.7209 26.2882 37.6164 25.1836 37.6164H22.8164C21.7118 37.6164 20.8164 36.7209 20.8164 35.6164V32.095Z" fill="var(--sapContent_Illustrative_Color5)"/>
<path d="M29.9482 22.2842C29.9482 21.1796 30.8437 20.2842 31.9482 20.2842H34.3154C35.42 20.2842 36.3154 21.1796 36.3154 22.2842V35.6165C36.3154 36.721 35.42 37.6165 34.3154 37.6165H31.9482C30.8437 37.6165 29.9482 36.721 29.9482 35.6165V22.2842Z" fill="var(--sapContent_Illustrative_Color3)"/>
</svg>
`;t("tnt/V5/Calculator",async function(){return(await r(async()=>{const{default:l}=await import("./Calculator-FzVgNuVV.js");return{default:l}},__vite__mapDeps([0,1]))).default});t("tnt/V5/HC/Calculator",async function(){return(await r(async()=>{const{default:l}=await import("./Calculator-tpZwJqPg.js");return{default:l}},__vite__mapDeps([2,1]))).default});const s="Calculator",v="tnt",_="V4";o(s,{dialogSvg:e,sceneSvg:i,spotSvg:n,dotSvg:C,set:v,collection:_});const p="tnt/Calculator";export{p as default,e as dialogSvg,C as dotSvg,i as sceneSvg,n as spotSvg};
